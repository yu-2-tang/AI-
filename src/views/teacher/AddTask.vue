...
export default {
  ...
  mounted() {
    ...
    if (saved) {
      this.generatedExam = JSON.parse(saved);
      if (this.generatedExam?.taskCode) {
        this.task.title = this.generatedExam.taskCode;
  ...
  // 使用 PreviewExam 的 params:id 路由参数
  this.$router.push({ name: 'PreviewExam', params: { id: 'temp-preview' } });
},
  ...
  alert('已清除生成的试卷');
    ...
    async saveTask() {
      ...
      try {
        ...
        // 如果是试卷任务，并且有预生成试卷数据，附加 questions
